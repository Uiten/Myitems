...
public class CommandLoreInsert extends HandlerCommand implements CommandExecutor {
   public CommandLoreInsert(MyItems plugin) {
      super(plugin);
   }
   ...
   protected static final boolean insertLore(CommandSender sender, Command command, String label, String[] args) {
      ...
      if (!commandManager.checkPermission(sender, "Lore_Insert")) {
         String permission = commandManager.getPermission("Lore_Insert");
         ...
         if (!EquipmentUtil.isSolid(item)) {
            ...
            if (args.length < 1) {
               textLine = TextUtil.getJsonTooltip(lang.getText(sender, "Tooltip_Lore_Insert"));
               message = lang.getMessage(sender, "Argument_InsertLore");
               message.sendMessage(sender, "tooltip_lore_insert", textLine);
               SenderUtil.playSound(sender, SoundEnum.ENTITY_BLAZE_DEATH);
               return true;
            } else {
               textLine = args[0];
               if (!MathUtil.isNumber(textLine)) {
                  ...
                  if (line < 1) {
                     ...
                     message = lang.getMessage(sender, "MyItems_InsertLore_Sucess");
