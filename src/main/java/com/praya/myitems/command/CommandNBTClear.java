...
public class CommandNBTClear extends HandlerCommand implements CommandExecutor {
   public CommandNBTClear(MyItems plugin) {
      super(plugin);
   }
   ...
   protected static final boolean clearNBT(CommandSender sender, Command command, String label, String[] args) {
      ...
      if (!commandManager.checkPermission(sender, "Attribute_NBT")) {
         String permission = commandManager.getPermission("Attribute_NBT");
         ...
         if (!EquipmentUtil.isSolid(item)) {
            ...
            message = lang.getMessage(sender, "MyItems_ClearNBT_Success");
