...
public class AbilityWeaponRoots extends AbilityWeapon implements AbilityWeaponAttributeBaseDamage, AbilityWeaponAttributeEffect {
   ...
   private AbilityWeaponRoots(MyItems plugin, String id) {
      super(plugin, id);
   }

   public static final AbilityWeaponRoots getInstance() {
      return AbilityWeaponRoots.AbilityRootsHelper.instance;
   }
   ...
   public int getMaxGrade() {
      ...
      AbilityWeaponProperties abilityWeaponProperties = abilityWeaponManager.getAbilityWeaponProperties("Roots");
   ...
   public void cast(Entity caster, Entity target, int grade, double damage) {
      ...
      if (target instanceof LivingEntity) {
         ...
         double seconds = (double)duration / 20.0D;
         MessageBuild messageAttacker = lang.getMessage(attacker, "Ability_Roots_Attacker");
         MessageBuild messageVictims = lang.getMessage(victims, "Ability_Roots_Victims");
         ...
         Bridge.getBridgeParticle().playParticle(players, ParticleEnum.SPELL, location, 10, 0.25D, 0.25D, 0.25D, 0.10000000149011612D);
         ...
         (new BukkitRunnable() {
            ...
            public void run() {
               if (this.time > duration) {
                  ...
                  Vector vector = victims.getVelocity().setY(-1);
   ...
   // $FF: synthetic method
   AbilityWeaponRoots(MyItems var1, String var2, AbilityWeaponRoots var3) {
      this(var1, var2);
   }
   ...
   private static class AbilityRootsHelper {
      ...
      static {
         MyItems plugin = (MyItems)JavaPlugin.getPlugin(MyItems.class);
         instance = new AbilityWeaponRoots(plugin, "Roots", (AbilityWeaponRoots)null);
      }
   }
}
