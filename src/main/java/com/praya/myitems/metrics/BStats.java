...
public class BStats {
   ...
   private final List<BStats.CustomChart> charts = new ArrayList();
   ...
   public JSONObject getPluginData() {
      ...
      while(var6.hasNext()) {
         BStats.CustomChart customChart = (BStats.CustomChart)var6.next();
   ...
   private void submitData() {
      ...
      (new Thread(new Runnable() {
         public void run() {
            try {
               BStats.sendData(data);
            } catch (Exception var2) {
               if (BStats.logFailedRequests) {
                  BStats.this.plugin.getLogger().log(Level.WARNING, "Could not submit plugin stats of " + BStats.this.plugin.getName(), var2);
               }
            }
